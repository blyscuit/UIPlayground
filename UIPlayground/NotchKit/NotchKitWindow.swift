...
//                    return 22
    ...
    private func updateSafeAreaInsets(animated: Bool = false) {
        ...
        let finalInsets = UIEdgeInsets(
            top:    maskedEdges.contains(.top)    ? insets.top - 20    : 0,
    ...
    private func updateStatusBar(animated: Bool = false) {
        if !isiPhoneX {
            return
        }
        UIApplication.shared.setStatusBarHidden(maskedEdges.contains(.top), with: animated ? .fade : .none)
        bringSubviewToFront(statusLabel)
        UIView.animate(withDuration: animated ? 0.3 : 0) {
            self.statusLabel.alpha = self.maskedEdges.contains(.top) ? 1 : 0
        }
    }
